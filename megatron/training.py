...
def setup_model_and_optimizer(model_provider_func):
    ...
    if args.deepspeed:
        ...
        # ALBERT - start
        global TRAIN_BATCH_SIZE, GAS
        TRAIN_BATCH_SIZE = model.train_batch_size()
        GAS = model.gradient_accumulation_steps()
        # ALBERT - end
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        ...
        # ALBERT: for experiments logs purposes - start
        if 20 <= iteration <= 100:
            elapsed_time_per_iteration_list.append(elapsed_time * 1000.0 / args.log_interval)
        # ALBERT: for experiments logs purposes - end
        ...
        for key in total_loss_dict:
            if key not in [skipped_iters_key, got_nan_key]:
                try:
                    tot_loss = total_loss_dict[key].item()
                except:
                    tot_loss = total_loss_dict[key]
                #avg = total_loss_dict[key].item() / float(num_iterations)
                avg = tot_loss / float(num_iterations)
    ...
    while iteration < args.train_iters:
        ...
        if iteration == 50:
            if torch.distributed.get_rank() == 0:
                ...
                assert pipe_parallel_size == 0, 'ALBERT: pipeline parallel size is not 0'
                data_parallel_size = torch.distributed.get_world_size() // model_parallel_size
                f = open(f'/hot/albert/global_bsz={TRAIN_BATCH_SIZE}_zero_stage={args.zero_stage}.txt', 'a')
                from statistics import mean
                f.write(f'until iteration {iteration}: '
                        f'global_bsz={TRAIN_BATCH_SIZE}_zero_stage={args.zero_stage}_dp={data_parallel_size}_mp={model_parallel_size}_pp={pipe_parallel_size}_gas={GAS}: '
                        f'avg_elapsed_time={mean(elapsed_time_per_iteration_list)}\n')
                f.close()
            # import time
            # time.sleep(3)
            # print(f'ALBERT DEBUG: DP RANK {mpu.get_data_parallel_rank()}: Shutting down!')
            # sys.exit()
